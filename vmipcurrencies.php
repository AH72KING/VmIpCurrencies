 ...
 * @subpackage  System.VmIpCurrencies 
    ...
    */
    ...
    {    
        ...
        $app = JFactory::getApplication();
            $user_ip = $_SERVER['REMOTE_ADDR']; // to get current ip address
            $details = unserialize(file_get_contents('http://www.geoplugin.net/php.gp?ip='.$user_ip));
            $country_code =$details['geoplugin_countryCode'];
/*
this is what is return from above url it can be used for other purpose but i am jus using country_code
//$user_ip='194.140.132.178'; //mexican city ip address for test
array (
...
*/
           ...
           $ipBaseCurrency=$country_code;
            if (!empty($ipBaseCurrency))
            {
                $ipBaseCurrency = $this->getCurrencyId($ipBaseCurrency);
